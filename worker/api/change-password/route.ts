...
export async function handleChangePassword(c: HandlerContext, env: Env) {
    ...
    if (!parsed.success) {
        return c.json({ message: 'Invalid request', errors: parsed.error.flatten().fieldErrors }, 400);
    }
    const { token, email, password } = parsed.data;
    const db = getDb(env.DB);

    const [verification] = await db.select().from(verifications).where(eq(verifications.id, token));
