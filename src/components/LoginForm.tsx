...
export default function LoginForm({ onLogin }: { onLogin: (user: AuthUser) => void }) {
  ...
  function handleLogin(val: AuthUser) {
    onLogin(val)
  }

  function doLogin(e: React.MouseEvent<HTMLButtonElement>) {
    e.preventDefault()
    if (!email || !password) {
          ...
          <form className="space-y-6">
            ...
            <div className="mt-4">
              <div className="flex items-center justify-between">
                ...
                <Link to="/forgot-password" tabIndex={-1} className="text-sm text-blue-500 hover:underline">Forgot password?</Link>
              </div>
              <div className="mt-1">
                <input
                  id="password"
