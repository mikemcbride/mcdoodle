...
import { Poll, Submission, Vote } from '../types';
...
const SubmissionForm = ({ poll, submission, handleCancel, handleSubmitted }: PropTypes) => {
  // set up local component data
  const { user } = useAuth();
  const questions = poll.questions;
  if (!questions || !questions.length) {
    console.error('Poll does not have any questions');
    return null;
  }
  ...
  const defaultSubmittedBy = submission === null ? (user?.firstName ? user.firstName : '') : (submission?.person || '');

  const [votes, setVotes] = useState(initialVotes)
  const [submittedBy, setSubmittedBy] = useState<string>(defaultSubmittedBy)
  const [submitting, setSubmitting] = useState<boolean>(false)
  const [showInvalidMessage, setShowInvalidMessage] = useState<boolean>(false)
  const [validationErrors, setValidationErrors] = useState<string[]>([])

  // methods
  function setVote(payload: Vote) {
    const existing = votes.find(vote => vote.question === payload.question)
    if (!existing) {
  ...
  function doValidation() {
    ...
    const emptyVotes = votes.filter(vote => !vote.response)
  ...
  function handleSubmit() {
    ...
    if (submission !== null) {
      ...
      const updatedResponses = submission.responses.map(r => {
        const newVal = votes.find(v => v.question === r.question_id)
        if (newVal) {
        ...
        <div className="md:rounded-lg shadow ring-1 ring-black/5 bg-white p-4 md:p-6 space-y-6 md:space-y-8">
          ...
          <footer className="flex justify-end items-center space-x-4 mt-8">
            ...
            <button onClick={handleSubmit}
              ...
              disabled={submitting}
            ><Spinner show={submitting} className="text-white h-4 w-4 mr-2" />{submitting ? 'Submitting' : 'Submit'}</button>
          </footer>
        </div>
      </div>
    </div>
