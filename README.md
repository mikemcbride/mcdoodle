...
You need to connect to Planetscale and then update the `.env` file to point to the database that's running locally:

```
pscale connect mcdoodle <branch-name> --port 3309
```

Then in another tmux session/terminal tab, you can run your app. The above command needs to be running while developing locally.
