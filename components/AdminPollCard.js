...
export default function AdminPollCard({ poll, onRemovePoll }) {
    ...
    <li>
      <Dialog open={isOpen} onClose={setIsOpen} className="absolute z-50">
        ...
        <div className="fixed inset-0 flex mt-24 items-start justify-center p-4">
          <Dialog.Panel className="w-full max-w-md rounded-lg shadow-md bg-white p-6">
            <Dialog.Title className="text-2xl font-bold block text-red-600">Slow down!</Dialog.Title>
            ...
            <footer className="mt-8 flex justify-end space-x-4">
              <button onClick={() => setIsOpen(false)} type="button" className="inline-flex items-center px-2.5 py-1.5 border border-gray-300 shadow-sm text-xs font-medium rounded text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">Cancel</button>
              <button onClick={() => deletePoll()} disabled={isDeleting} type="button" className="inline-flex items-center px-2.5 py-1.5 border border-transparent text-xs font-medium rounded text-red-700 bg-red-100 hover:bg-red-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">{isDeleting && <Spinner className="h-4 w-4 mr-2 inline-block" />}Delete</button>
            </footer>
          </Dialog.Panel>
        </div>
      </Dialog>
